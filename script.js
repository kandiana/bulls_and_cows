...
const BASIC_NUMBER_OF_DIGITS = 4

const possibleDigits = []

const gameState = {
	numberOfDigits: BASIC_NUMBER_OF_DIGITS,
...
function guess() {
	...
	const bullsAndCows = compareNumbers(gameState.numberToGuess, playerInputNumber.split('').map(Number))
	gameState.numberOfTriesUsed += 1

	updatePossibleDigits(bullsAndCows[0], bullsAndCows[1], playerInputNumber)

	addLineToPreviousTriesTable([gameState.numberOfTriesUsed, playerInputNumber, bullsAndCows[0], bullsAndCows[1]])
...
resetSettingsButton.addEventListener('click', () => {
	gameState.numberOfDigits = BASIC_NUMBER_OF_DIGITS
	digitsSettings.innerText = BASIC_NUMBER_OF_DIGITS
